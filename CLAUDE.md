...
- **公式サイトの結果ページ本文から機械的に読み取れるのは、プレゼン大会進出作(部門ベスト10のうち上位3冊)と各賞だけ**。ベスト10の全リストは画像として掲載されているため本文に出てこない。登録しているのはこの2種類のみで、`result`は「技術書部門ベスト10(プレゼン大会進出)」「部門大賞」「特別賞」のような文字列
...
893冊(2026-08-06 時点)。著者995・技術スタック106・翻訳者280・出版社68・テーマ13・アワード1。
表紙は796/893(89%)が解決済み。未解決は絶版で楽天・Koboに在庫がない古い本が中心。
