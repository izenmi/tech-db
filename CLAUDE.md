...
好きな技術を最大3つ選ぶと、技術の重なりが近い本を一致度つきで20冊出す1ページ
(`src/ui/recommend/RecommendPage.tsx`)。

- **スコアは「この本が好きなら」と同じ式**をクライアント側で再現している(`idf(t)=log(N/df(t))` の
  IDF重み付きコサイン類似度)。**別の式を新造しないこと** — 詳細ページの並びと食い違うため。
  制作者ボーナスは種になる本がないので足さず、そのぶん上限が1.0になり「一致度◯%」として出せる
- **同点が支配的になる**(選んだ技術だけを持つ本は全部100%)。id昇順だけだと
  スラッグ順という無意味な並びになるので、受賞件数→受賞件数→id昇順で崩している
- **データは `generated/recommend-index.json`**(技術表 + 各本の技術idだけ)。
  これだけで選択UIが動き、**works.json は技術を1つ選んでから**取りに行く。
  **この索引の読み手は /recommend だけ。ページを消すなら生成も消すこと**。**このサイトだけテーマ(13件)ではなく技術スタック(106件)を選択軸にしている**(テーマが軸として薄いため)
